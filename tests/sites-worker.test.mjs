...
test("Architectural Glass remains centralized, responsive and accessible", async () => {
  const [tokens, glass, motion, shell] = await Promise.all([
    readFile(new URL("../src/styles/tokens.css", import.meta.url), "utf8"),
    readFile(
      new URL("../src/styles/architectural-glass.css", import.meta.url),
      "utf8",
    ),
    readFile(new URL("../src/styles/motion.css", import.meta.url), "utf8"),
    readFile(new URL("../src/layouts/SiteShell.astro", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(glass, /@supports[\s\S]*backdrop-filter/);
  assert.match(glass, /\.button:focus-visible/);
  assert.match(glass, /min-height:\s*48px/);
  assert.match(glass, /@media \(max-width:\s*520px\)/);
  assert.match(motion, /prefers-reduced-motion:\s*reduce/);
  assert.doesNotMatch(glass, /neon|#00ffff|#ff00ff/i);
  assert.match(shell, /styles\/tokens\.css/);
  assert.match(shell, /styles\/architectural-glass\.css/);
  assert.match(shell, /styles\/motion\.css/);
});

test("SEO GEO language and consent signals survive the redesign", async () => {
  const [shell, analytics, robots, sitemap] = await Promise.all([
    readFile(new URL("../src/layouts/SiteShell.astro", import.meta.url), "utf8"),
    readFile(new URL("../src/analytics.js", import.meta.url), "utf8"),
    readFile(new URL("../public/robots.txt", import.meta.url), "utf8"),
    readFile(new URL("../src/pages/sitemap.xml.js", import.meta.url), "utf8"),
  ]);
  assert.match(shell, /rel="canonical"/);
  assert.match(shell, /hreflang/);
  assert.match(shell, /application\/ld\+json/);
  assert.match(shell, /localeConfig\.direction/);
  assert.match(analytics, /getAnalyticsConsent\(\) !== "granted"/);
  assert.match(robots, /OAI-SearchBot/);
  assert.match(sitemap, /enabledLocales/);
});
