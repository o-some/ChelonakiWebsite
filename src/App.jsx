...
function Header({ openMenu, path }) {
  ...
  const [dismissedGroup, setDismissedGroup] = useState(null);
...
function HubPage({ type }) {
  ...
  return <main id="main"><section className={`page-hero page-hero-${type}`}><div><span className="audience-label"><Buildings size={15}/>{data.label}</span><h1>{data.title}</h1><p>{data.intro}</p><div className="page-hero-actions"><SmartLink className="button button-gold" href={`/kontakt?bereich=${type}`}>{data.cta}<ArrowRight size={18}/></SmartLink><SmartLink className="text-link text-link-light" href="/paketfinder">Passende Lösung finden <ArrowRight size={18}/></SmartLink></div></div><figure><img src={data.image} alt="Chelonaki Markenwelt"/></figure></section><section className="service-index"><header><span>Entdecken</span><h2>Womit möchten Sie beginnen?</h2></header><div>{data.cards.map((card, i) => <SmartLink className="service-index-card" href={card.href} key={card.href}><span>0{i+1}</span><h3>{card.title}</h3><p>{card.text}</p><ArrowUpRight size={21}/></SmartLink>)}</div></section>{type === "expertise" && <Credentials/>}<Process/><FinalCta label={data.cta}/></main>;
}

function ServicePage({ data }) {
  return <main id="main"><section className="service-hero"><div className="service-hero-copy"><span className="audience-label"><Buildings size={15}/>{data.label}</span><small>{data.area}</small><h1>{data.title}</h1><p>{data.intro}</p><div className="page-hero-actions"><SmartLink className="button button-gold" href={`/kontakt?bereich=${encodeURIComponent(data.bookingCta || data.cta)}`}>{data.bookingCta || data.cta}<ArrowRight size={18}/></SmartLink><SmartLink className="text-link text-link-light" href="/paketfinder">Paketfinder nutzen <ArrowRight size={18}/></SmartLink></div>{data.bookingCta && <p className="booking-status">Die direkte Shopify-Buchung wird nach Anlage der finalen Produkte und rechtlichen Freigaben aktiviert.</p>}</div><figure><img src={data.area === "Digital" ? "/assets/project-digital-brand.png" : "/assets/chapter-architecture.webp"} alt="Chelonaki Projektansicht"/></figure></section><section className="problem-solution"><div><span>Ausgangssituation</span><h2>{data.problem}</h2></div><div><span>Die Lösung</span><ul>{data.solution.map((item) => <li key={item}><Check size={18}/>{item}</li>)}</ul></div></section>{data.insight && <section className="platform-section"><div><span>{data.insight.label}</span><h2>{data.insight.title}</h2><p>{data.insight.text}</p></div><ul>{data.insight.points.map((point) => <li key={point}><Check size={18}/>{point}</li>)}</ul></section>}<Process items={data.steps}/><ProjectReadiness data={data}/><Pricing data={data}/><FinalCta label={data.inquiryCta || data.cta}/></main>;
}
...
function AcademyPage() {
  return <main id="main"><section className="service-hero"><div className="service-hero-copy"><span className="audience-label"><Buildings size={15}/>KI-Beratung & Weiterbildung</span><small>Chelonaki Video Academy</small><h1>KI-Wissen lernen, wenn es in Ihren Alltag passt.</h1><p>Strukturierte Videokurse führen Schritt für Schritt von den Grundlagen zu anwendbaren Workflows – mit Beispielen, Vorlagen und klaren Qualitätsregeln.</p><div className="page-hero-actions"><SmartLink className="button button-gold" href="/kontakt?bereich=Chelonaki%20Video%20Academy">Zugang vormerken <ArrowRight size={18}/></SmartLink></div></div><figure><img src="/assets/chapter-architecture.webp" alt="Architektonische Lernstruktur"/></figure></section><section className="choice-grid"><article><span>Für Mitarbeiter und professionelle Anwender</span><h2>KI-Videokurse</h2><p>Werkzeuge sinnvoll auswählen, wirksam prompten, Ergebnisse prüfen, Abläufe automatisieren und verantwortungsvoll freigeben.</p></article><article><span>Lernweg mit Praxisbezug</span><h2>Vom Modul zur Anwendung</h2><p>Kurze Lerneinheiten, konkrete Übungen, wiederverwendbare Vorlagen und verständliche Zusammenfassungen unterstützen den Transfer.</p></article></section><section className="release-note"><strong>In Vorbereitung</strong><p>Kursumfang, Verkauf, Lernkontrollen, Teilnahmebestätigungen und mögliche Zertifikate werden vor Freigabe abschließend festgelegt und rechtlich geprüft.</p></section><FinalCta label="Video Academy vormerken"/></main>;
}
...
function OriginalDetail({ type }) {
  ...
  return <main id="main"><section className="original-hero"><span className="audience-label">Eleftherios · Eigenes Werk</span><h1>{data[0]}</h1><p>{data[1]}</p><div>{data[2].map((item) => <span key={item}>{item}</span>)}</div><SmartLink className="button button-gold" href={`/kontakt?bereich=${encodeURIComponent(data[0])}`}>Warteliste & Partnerschaft <ArrowRight size={18}/></SmartLink></section><section className="release-note"><strong>Eigene Werke & Entwicklungsstände</strong><p>Originals zeigt veröffentlichte, geplante und kommende eigene Produkte von Eleftherios. Medizinische und ernährungsbezogene Inhalte werden quellenbasiert erstellt und vor Veröffentlichung fachlich geprüft.</p></section></main>;
}
