...
const labEntries = [
  { title: "Webdesign-Beispiele", status: "Als Vorlage wählbar", area: "Websites & Shops", text: "Wählen Sie eine visuelle Richtung für Aufbau, Typografie, Farben und Bildsprache. Jede Vorlage wird anschließend individuell auf Ihre Marke, Inhalte und Ziele zugeschnitten.", image: "/assets/design-web.png", href: "/web-apps-publikationen/websites-erstellen-lassen" },
  { title: "Content-Designbeispiele", status: "Als Vorlage wählbar", area: "Content & Social Media", text: "Vergleichen Sie unterschiedliche Gestaltungswelten für Beiträge, Karussells und Kurzvideos – als Ausgangspunkt für einen konsistenten Markenauftritt.", image: "/assets/design-content.png", href: "/medien-ai/social-media" },
  { title: "Anzeigen-Designbeispiele", status: "Als Vorlage wählbar", area: "Meta & Google Ads", text: "Entdecken Sie Creative-Richtungen für aufmerksamkeitsstarke Anzeigen. Motive, Formate und Botschaften werden für Ihr Angebot neu entwickelt.", image: "/assets/design-ads.png", href: "/medien-ai/ads" },
  { title: "Telefonassistent-Beispiele", status: "Ablauf als Vorlage", area: "KI-Telefonassistenten", text: "Sehen Sie beispielhafte Gesprächswege für Begrüßung, Anliegen, Terminübergabe und Eskalation. Die spätere Logik wird auf Ihren Betrieb angepasst.", image: "/assets/design-phone.png", href: "/medien-ai/ki-telefon" },
  { title: "App-Designbeispiele", status: "Als Vorlage wählbar", area: "Apps & Software", text: "Wählen Sie eine Richtung für Navigation, Dashboards und Nutzerführung. Funktionen und Oberfläche entstehen danach passend zu Ihrem Produkt.", image: "/assets/design-apps.png", href: "/web-apps-publikationen/apps-entwickeln-lassen" },
  { title: "Buchdesign-Beispiele", status: "Als Vorlage wählbar", area: "Bücher & Ghostwriting", text: "Cover, Kapitelauftakt, Rezeptseite oder Fachbuchlayout: Markieren Sie den Stil, der zu Ihrem Thema und Ihrer Zielgruppe passt.", image: "/assets/design-books.png", href: "/web-apps-publikationen/buecher-erstellen-lassen-ghostwriting" },
  { title: "Reply-Oberflächenbeispiele", status: "Als Vorlage wählbar", area: "KI-Antworten", text: "Vergleichen Sie ruhige, kompakte Bedienkonzepte für Antwortvorschläge und menschliche Freigabe auf Social Media und im E-Mail-Alltag.", image: "/assets/design-reply.png", href: "/medien-ai/chelonaki-reply" },
  { title: "Lernwelt-Beispiele", status: "Als Vorlage wählbar", area: "Academy & Schulung", text: "Wählen Sie eine Lernwelt für Kurse, Module, Fortschritt und Arbeitsmaterialien. Didaktik und Inhalte werden separat auf Ihr Ziel abgestimmt.", image: "/assets/design-training.png", href: "/ki-beratung-weiterbildung/video-academy" },
  { title: "Beratungswelten", status: "Ablauf als Vorlage", area: "KI-Beratung", text: "Beispielhafte Workshop- und Strategieformate zeigen, wie Analyse, Priorisierung und Roadmap visuell verständlich aufgebaut werden können.", image: "/assets/design-consulting.png", href: "/ki-beratung-weiterbildung/ki-beratung-unternehmen" },
  { title: "Originals-Präsentationen", status: "Inspiration", area: "Eigene Apps & Bücher", text: "Entdecken Sie, wie eigene Apps und Bücher hochwertig präsentiert werden können – als Inspiration für Produktseiten und persönliche Markenwelten.", image: "/assets/design-originals.png", href: "/originals" },
];
...
function LabPage() {
  ...
  return <main id="main"><section className="lab-hero"><div><span>Chelonaki Designbibliothek</span><h1>Eine Richtung wählen.<br/>Etwas Eigenes daraus machen.</h1><p>Hier finden Sie kuratierte Design- und Ablaufbeispiele für Websites, Apps, Inhalte, Anzeigen, Bücher und Lernwelten. Wählen Sie, was Ihnen gefällt – wir entwickeln daraus eine individuelle Lösung für Ihre Marke.</p><div className="page-hero-actions"><a className="button button-gold" href="#lab-demos">Designbeispiele ansehen <ArrowRight size={18}/></a><SmartLink className="text-link text-link-light" href="/paketfinder">Passende Leistung finden <ArrowRight size={18}/></SmartLink></div></div><div className="lab-hero-side"><figure><img src="/assets/design-web.png" alt="Mediterranes Designatelier mit digitalen Entwürfen" width="1536" height="1024"/></figure><aside><strong>{String(saved.length).padStart(2, "0")}</strong><span>Beispiele in Ihrer Auswahl</span><p>Merken Sie mehrere Richtungen. Im Gespräch verbinden wir die passenden Merkmale zu Ihrem individuellen Design.</p></aside></div></section><section className="lab-how"><header><span>So funktioniert die Auswahl</span><h2>Inspiration ohne Baukasten-Look.</h2></header><ol><li><strong>01</strong><h3>Ansehen</h3><p>Vergleichen Sie Stil, Wirkung und Nutzerführung.</p></li><li><strong>02</strong><h3>Merken</h3><p>Speichern Sie die Richtungen, die zu Ihrer Idee passen.</p></li><li><strong>03</strong><h3>Individualisieren</h3><p>Wir entwickeln daraus eine eigenständige Lösung für Marke, Zielgruppe und Inhalt.</p></li></ol></section><section className="lab-grid-section" id="lab-demos"><header><span>Design- und Ablaufbeispiele</span><h2>Wählen Sie die Richtung, nicht das fertige Endprodukt.</h2><p>Die Beispiele dienen als visuelle und strukturelle Vorlage. Texte, Farben, Funktionen, Bilder und Details werden für Ihr Projekt individuell entwickelt.</p></header><div className="lab-grid">{labEntries.map((entry, i) => <article key={entry.title}><figure><img src={entry.image} alt="" loading="lazy" width="1536" height="1024"/></figure><div className="lab-card-meta"><span>{entry.status}</span><small>{entry.area}</small></div><strong>{String(i + 1).padStart(2, "0")}</strong><h3>{entry.title}</h3><p>{entry.text}</p><div><SmartLink href={entry.href}>Passenden Bereich öffnen <ArrowUpRight size={17}/></SmartLink><button type="button" aria-pressed={saved.includes(entry.title)} onClick={() => toggle(entry.title)}>{saved.includes(entry.title) ? "Ausgewählt" : "Als Vorlage merken"}</button></div></article>)}</div></section><section className="lab-transfer"><div><span>Ihre Designauswahl</span><h2>Welche Richtung passt zu Ihrer Idee?</h2><p>{saved.length ? `${saved.length} Beispiel${saved.length === 1 ? " ist" : "e sind"} ausgewählt. Nutzen Sie den Paketfinder oder nennen Sie uns Ihre Auswahl im Erstgespräch.` : "Merken Sie passende Beispiele. Danach können Sie die richtige Leistung finden oder Ihre bevorzugte Richtung direkt mit uns besprechen."}</p></div><SmartLink className="button button-gold" href="/paketfinder">Mit Auswahl weiter <ArrowRight size={18}/></SmartLink></section></main>;
}
